...
void tray_init(CefRefPtr<CefBrowser> browser, CefRefPtr<CefWindow> window) {
    ...
    if (len > 0) {
        ...
        for (const auto& candidate : {
            exe_dir + "/ofl.svg",
            exe_dir + "/../data/ofl.svg",
            exe_dir + "/../share/icons/ofl.svg",
            std::string("/usr/share/icons/hicolor/scalable/apps/ofl.svg"),
        }) {
    ...
    app_indicator_set_title(g_indicator, "Outlook Lite for Linux");
    ...
    fprintf(stderr, "[ofl] Tray initialized\n");
}
...
void tray_shutdown() {
    ...
    g_browser = nullptr;
    g_window = nullptr;
    fprintf(stderr, "[ofl] Tray shutdown\n");
}
